...
public class CdiTaskListener implements TaskListener, Serializable {
    ...
    protected BusinessProcessEvent createEvent(DelegateTask task) {
        ProcessDefinition processDefinition = ProcessDefinitionCacheUtil.getCachedProcessDefinitionEntity(task.getExecution()
                .getProcessDefinitionId());
        return new CdiBusinessProcessEvent(activityId, transitionName, processDefinition, task, type, task.getExecution()
                .getProcessInstanceId(), task.getExecutionId(), new Date());
    }
