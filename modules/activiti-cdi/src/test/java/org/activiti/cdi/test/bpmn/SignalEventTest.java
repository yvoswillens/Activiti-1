...
public class SignalEventTest extends CdiActivitiTestCase {
    ...
    public static class SendSignalDelegate implements JavaDelegate {
        ...
        public void execute(DelegateExecution execution) {
            ...
            String executionId = runtimeService.createExecutionQuery().processInstanceId(signalProcessInstanceId)
                    .signalEventSubscriptionName("alert").singleResult().getId();
    ...
    public void testSignalCatchBoundaryWithVariables() {
        ...
        assertEquals(1,
                runtimeService.createExecutionQuery().processInstanceId(piCatchSignal.getProcessInstanceId()).activityId("receiveTask")
                        .count());
        assertEquals(1,
                runtimeService.createExecutionQuery().processInstanceId(piThrowSignal.getProcessInstanceId()).activityId("receiveTask")
                        .count());
