...
public class ErrorThrowingEventListenerTest extends PluggableActivitiTestCase {
    ...
    public void testThrowError() throws Exception {
        ...
        try {
            ...
            // dispatched
            Task task = taskService.createTaskQuery().processInstanceId(processInstance.getId()).taskDefinitionKey("userTask")
                    .singleResult();
            assertNotNull(task);
            ...
            task = taskService.createTaskQuery().processInstanceId(processInstance.getId()).taskDefinitionKey("escalatedTask")
                    .singleResult();
            assertNotNull(task);
    ...
    public void testThrowErrorWithErrorcode() throws Exception {
        ...
        try {
            ...
            task = taskService.createTaskQuery().processInstanceId(processInstance.getId()).taskDefinitionKey("escalatedTask2")
                    .singleResult();
            assertNotNull(task);
        } finally {
            processEngineConfiguration.getEventDispatcher().removeEventListener(listener);
