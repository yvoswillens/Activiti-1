...
public class StartAuthorizationTest extends PluggableActivitiTestCase {
    ...
    public void testIdentityLinks() throws Exception {
        ...
        try {
            ProcessDefinition latestProcessDef = repositoryService.createProcessDefinitionQuery().processDefinitionKey("process1")
                    .singleResult();
    ...
    public void testAddAndRemoveIdentityLinks() throws Exception {
        ...
        try {
            ProcessDefinition latestProcessDef = repositoryService.createProcessDefinitionQuery()
                    .processDefinitionKey("potentialStarterNoDefinition").singleResult();
            assertNotNull(latestProcessDef);
            List<IdentityLink> links = repositoryService.getIdentityLinksForProcessDefinition(latestProcessDef.getId());
            assertEquals(0, links.size());
    ...
    public void testPotentialStarter() throws Exception {
        ...
        try {
            ...
            ProcessDefinition potentialStarter = repositoryService.createProcessDefinitionQuery().processDefinitionKey("potentialStarter")
                    .startableByUser("user1").latestVersion().singleResult();
            assertNotNull(potentialStarter);

            potentialStarter = repositoryService.createProcessDefinitionQuery().processDefinitionKey("potentialStarter")
                    .startableByUser("user3").latestVersion().singleResult();
            assertNotNull(potentialStarter);

            potentialStarter = repositoryService.createProcessDefinitionQuery().processDefinitionKey("potentialStarter")
                    .startableByUser("userInGroup2").latestVersion().singleResult();
            assertNotNull(potentialStarter);

            potentialStarter = repositoryService.createProcessDefinitionQuery().processDefinitionKey("potentialStarter")
                    .startableByUser("userInGroup3").latestVersion().singleResult();
            assertNotNull(potentialStarter);
        } finally {
    ...
    public void testProcessDefinitionList() throws Exception {
        ...
        try {
            ...
            List<ProcessDefinition> processDefinitions = repositoryService.createProcessDefinitionQuery().orderByProcessDefinitionName()
                    .asc().list();
            ...
            processDefinitions = repositoryService.createProcessDefinitionQuery().orderByProcessDefinitionName().asc()
