...
public class DefaultAsyncJobExecutor implements AsyncExecutor {
    ...
    protected void startExecutingAsyncJobs() {
        ...
        if (executorService == null) {
            log.info("Creating executor service with corePoolSize {}, maxPoolSize {} and keepAliveTime {}", corePoolSize, maxPoolSize,
                    keepAliveTime);
    ...
    protected void stopExecutingAsyncJobs() {
        ...
        try {
            if (!executorService.awaitTermination(secondsToWaitOnShutdown, TimeUnit.SECONDS)) {
                log.warn("Timeout during shutdown of async job executor. " + "The current running jobs could not end within "
