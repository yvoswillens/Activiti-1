...
public class JobRetryCmd implements Command<Object> {
        ...
        if (activity == null || activity.getFailedJobRetryTimeCycleValue() == null) {
            ...
            try {
                ...
                if (job.getExceptionMessage() == null) { // is it the first
                                                         // exception
                    log.debug("Applying JobRetryStrategy '" + failedJobRetryTimeCycle + "' the first time for job " + job.getId()
                            + " with " + durationHelper.getTimes() + " retries");
    ...
    private ActivityImpl getCurrentActivity(CommandContext commandContext, JobEntity job) {
        ...
        if (TimerExecuteNestedActivityJobHandler.TYPE.equals(type) || TimerCatchIntermediateEventJobHandler.TYPE.equals(type)) {
            ...
            ProcessDefinitionEntity processDefinition = deploymentManager.findDeployedLatestProcessDefinitionByKeyAndTenantId(
                    job.getJobHandlerConfiguration(), job.getTenantId());
