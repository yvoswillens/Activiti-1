...
public class EntityManagerSessionImpl implements EntityManagerSession {
    ...
    public EntityManagerSessionImpl(EntityManagerFactory entityManagerFactory, EntityManager entityManager, boolean handleTransactions,
    ...
    public void close() {
        if (closeEntityManager && entityManager != null && !entityManager.isOpen()) {
            try {
                ...
                throw new ActivitiException("Error while closing EntityManager, may have already been closed or it is container-managed",
                        ise);
            }
        }
    }
