    ...
    protected mxRectangle apply(TreeNode node, mxRectangle bounds) {
        ...
        if (node.cell != null && g != null) {
            ...
            if (bounds == null) {
                ...
                bounds = new mxRectangle(Math.min(bounds.getX(), g.getX()), Math.min(bounds.getY(), g.getY()), Math.max(bounds.getX()
                        + bounds.getWidth(), g.getX() + g.getWidth()), Math.max(bounds.getY() + bounds.getHeight(),
                        g.getY() + g.getHeight()));
            }
        }
