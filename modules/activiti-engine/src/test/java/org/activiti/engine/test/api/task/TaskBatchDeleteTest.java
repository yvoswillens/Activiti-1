...
public class TaskBatchDeleteTest extends PluggableActivitiTestCase {
    ...
    public void testDeleteTaskWithChildren() throws Exception {
        ...
        Task firstTask = taskService.createTaskQuery().processInstanceId(processInstance.getId()).taskDefinitionKey("taskOne")
                .singleResult();
    ...
    public void testDeleteCancelledMultiInstanceTasks() throws Exception {
        ...
        Task lastTask = taskService.createTaskQuery().processInstanceId(processInstance.getId()).taskDefinitionKey("multiInstance")
                .listPage(4, 1).get(0);

        taskService.addCandidateGroup(lastTask.getId(), "sales");

        Task firstTask = taskService.createTaskQuery().processInstanceId(processInstance.getId()).taskDefinitionKey("multiInstance")
                .listPage(0, 1).get(0);
        assertNotNull(firstTask);
