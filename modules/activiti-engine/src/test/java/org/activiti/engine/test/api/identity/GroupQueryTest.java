...
public class GroupQueryTest extends PluggableActivitiTestCase {
    ...
    public void testNativeQuery() {
        ...
        assertEquals(1, identityService.createNativeGroupQuery().sql(baseQuerySql + " where ID_ = #{id}").parameter("id", "admin").list()
                .size());
        ...
        assertEquals(
                ...
                identityService
                        ...
                        .sql("SELECT aig.* from " + tableName + " aig" + " inner join ACT_ID_MEMBERSHIP aim on aig.ID_ = aim.GROUP_ID_ "
                                + " inner join ACT_ID_USER aiu on aiu.ID_ = aim.USER_ID_ where aiu.ID_ = #{id}").parameter("id", "kermit")
                        .list().size());
        ...
        assertEquals(1, identityService.createNativeGroupQuery().sql(baseQuerySql + " where ID_ = #{id}").parameter("id", "admin")
                .listPage(0, 1).size());
    }
