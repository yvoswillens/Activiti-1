...
public class TaskEntityManager extends AbstractEntityManager<TaskEntity> {
                ...
                commandContext
                        ...
                        .dispatchEvent(
                                ActivitiEventBuilder.createActivityCancelledEvent(task.getExecution().getActivityId(), task.getName(),
                                        task.getExecutionId(), task.getProcessInstanceId(), task.getProcessDefinitionId(), "userTask",
                                        UserTaskActivityBehavior.class.getName(), deleteReason));
            }
    ...
    public List<Task> findTasksAndVariablesByQueryCriteria(TaskQueryImpl taskQuery) {
        ...
        List<Task> instanceList = getDbSqlSession().selectListWithRawParameterWithoutFilter(query, taskQuery, taskQuery.getFirstResult(),
