...
public class ReportingUtil {
    ...
    public static ProcessDefinition getProcessDefinition(DelegateExecution delegateExecution) {
        ...
        if (executionEntity.getProcessDefinition() != null) {
            ...
            return ProcessEngines.getDefaultProcessEngine().getRepositoryService().createProcessDefinitionQuery()
                    .processDefinitionId(delegateExecution.getProcessDefinitionId()).singleResult();
        }
    }
    ...
    public static void generateTaskDurationReport(String processDefinitionId) {
        ...
        // Report descriptin
        String reportDescription = "Average task duration report for process definition " + processDefinition.getName() + " ( version "
        ...
        String script = "importPackage(java.sql);"
                ...
                + processDefinitionId + "' and END_TIME_ is not null group by NAME_\");" + "" + "var reportData = new ReportData();"
                + "var dataset = reportData.newDataset();" + "dataset.type = 'pieChart';" + "dataset.description = '" + reportDescription
                + "';" + "" + "while (result.next()) { " + "  var name = result.getString(1);" + "  var val = result.getLong(2) / 1000;"
                + "  dataset.add(name, val);" + "}" + "" + "execution.setVariable('reportData', reportData.toBytes());";

        // Generate bpmn model
        WorkflowDefinition workflowDefinition = new WorkflowDefinition().name(processDefinition.getName() + " task duration report")
                .description(reportDescription).addScriptStep(script);
        ...
        // Deploy
        repositoryService.createDeployment().name(processDefinition.getName() + " - task duration report")
