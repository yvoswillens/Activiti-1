...
public class EventOverviewPanel extends DetailPanel {
    ...
    protected void initializeCurrentValues() {
        if (ExplorerApp.get().getCrystalBallSimulationDebugger() != null) {
            ...
            List<HistoricProcessInstance> replayProcessInstanceList = historyService.createHistoricProcessInstanceQuery()
                    .processInstanceBusinessKey(SIMULATION_BUSINESS_KEY).orderByProcessInstanceStartTime().desc().list();
        ...
        replayButton.addListener(new ClickListener() {
            ...
            public void buttonClick(ClickEvent event) {
                if (instanceTable.getValue() != null) {
                    ...
                    SimpleEventCalendar eventCalendar = new SimpleEventCalendar(ProcessEngines.getDefaultProcessEngine()
                            .getProcessEngineConfiguration().getClock(), new SimulationEventComparator());
                    eventCalendar.addEvents(simulationEvents);

                    // replay process instance run
                    simulationDebugger = new ReplaySimulationRun(ProcessEngines.getDefaultProcessEngine(), eventCalendar,
    ...
    protected void initInstancesTable() {
        if (instanceList == null || instanceList.isEmpty()) {
            ...
            instanceTable.addContainerProperty("id", String.class, null, i18nManager.getMessage(Messages.PROCESS_INSTANCE_ID), null,
                    Table.ALIGN_LEFT);
            instanceTable.addContainerProperty("definitionName", String.class, null,
                    i18nManager.getMessage(Messages.PROCESS_INSTANCE_NAME), null, Table.ALIGN_LEFT);
            instanceTable.addContainerProperty("started", String.class, null, i18nManager.getMessage(Messages.PROCESS_INSTANCE_STARTED),
                    null, Table.ALIGN_LEFT);
            instanceTable.addContainerProperty("ended", String.class, null, i18nManager.getMessage(Messages.PROCESS_INSTANCE_ENDED), null,
                    Table.ALIGN_LEFT);
    ...
    protected void refreshInstances(String processDefinitionId) {
        instanceList = historyService.createHistoricProcessInstanceQuery().processDefinitionId(processDefinitionId)
    ...
    protected void initEventsTable() {
        ...
        eventTable.addContainerProperty("type", String.class, null, i18nManager.getMessage(Messages.CRYSTALBALL_EVENT_TYPE), null,
                Table.ALIGN_LEFT);
        eventTable.addContainerProperty("executed", String.class, null, i18nManager.getMessage(Messages.CRYSTALBALL_EVENT_EXECUTED), null,
                Table.ALIGN_LEFT);
    ...
    protected void fillEventValues() {
        ...
        if (replayHistoricInstance != null && replayHistoricInstance.getId() != null) {
            ProcessInstance testInstance = runtimeService.createProcessInstanceQuery().processInstanceId(replayHistoricInstance.getId())
                    .singleResult();
    ...
    protected List<Function<EventLogEntry, SimulationEvent>> getTransformers() {
        ...
        transformers.add(new EventLogProcessInstanceCreateTransformer(PROCESS_INSTANCE_START_EVENT_TYPE, PROCESS_DEFINITION_ID_KEY,
                SIMULATION_BUSINESS_KEY, VARIABLES_KEY));
    ...
    protected Map<String, SimulationEventHandler> getReplayHandlers(String processInstanceId) {
        ...
        handlers.put(PROCESS_INSTANCE_START_EVENT_TYPE, new StartReplayLogEventHandler(processInstanceId, PROCESS_DEFINITION_ID_KEY,
                SIMULATION_BUSINESS_KEY, VARIABLES_KEY));
