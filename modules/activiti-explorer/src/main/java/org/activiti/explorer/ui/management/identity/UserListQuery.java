...
public class UserListQuery extends AbstractLazyLoadingQuery {
    ...
    public List<Item> loadItems(int start, int count) {
        List<User> users = identityService.createUserQuery().orderByUserFirstName().asc().orderByUserLastName().asc().orderByUserId().asc()
    ...
    class UserListItem extends PropertysetItem implements Comparable<UserListItem> {
        ...
        public UserListItem(User user) {
            ...
            addItemProperty("name", new ObjectProperty<String>(user.getFirstName() + " " + user.getLastName() + " (" + user.getId() + ")",
                    String.class));
        }
