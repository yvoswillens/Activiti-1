...
public class TaskEventTextResolver implements Serializable {
    ...
    public Label resolveText(Event event) {
        ...
        String eventAuthor = "<span class='" + ExplorerLayout.STYLE_TASK_EVENT_AUTHOR + "'>" + user.getFirstName() + " "
        ...
        if (Event.ACTION_ADD_USER_LINK.equals(event.getAction())) {
            ...
            text = i18nManager.getMessage(Messages.EVENT_ADD_USER_LINK, eventAuthor,
                    involvedUser.getFirstName() + " " + involvedUser.getLastName(), event.getMessageParts().get(1)); // second
                                                                                                                     // msg
                                                                                                                     // part
                                                                                                                     // =
                                                                                                                     // role
            ...
            text = i18nManager.getMessage(Messages.EVENT_DELETE_USER_LINK, eventAuthor,
                    involvedUser.getFirstName() + " " + involvedUser.getLastName(), event.getMessageParts().get(1));
        } else if (Event.ACTION_ADD_GROUP_LINK.equals(event.getAction())) {
            text = i18nManager.getMessage(Messages.EVENT_ADD_GROUP_LINK, eventAuthor, event.getMessageParts().get(0), event
                    .getMessageParts().get(1)); // second msg part = role
        } else if (Event.ACTION_DELETE_GROUP_LINK.equals(event.getAction())) {
            text = i18nManager.getMessage(Messages.EVENT_DELETE_GROUP_LINK, eventAuthor, event.getMessageParts().get(0), event
                    .getMessageParts().get(1)); // second msg part = role
