...
public class DeleteDeploymentPopupWindow extends PopupWindow {
    ...
    protected void addDeleteWarning() {
        List<ProcessDefinition> processDefinitions = repositoryService.createProcessDefinitionQuery().deploymentId(deployment.getId())
                .list();
        ...
        if (nrOfProcessInstances == 0) {
            ...
            Label warningLabel = new Label(i18nManager.getMessage(Messages.DEPLOYMENT_DELETE_POPUP_WARNING, nrOfProcessInstances),
                    Label.CONTENT_XHTML);
