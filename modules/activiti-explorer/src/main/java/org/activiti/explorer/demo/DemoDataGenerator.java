...
public class DemoDataGenerator implements ModelDataJsonConstants {
    ...
    protected void initDemoUsers() {
        createUser("kermit", "Kermit", "The Frog", "kermit", "kermit@activiti.org", "org/activiti/explorer/images/kermit.jpg",
                Arrays.asList("management", "sales", "marketing", "engineering", "user", "admin"), Arrays.asList("birthDate", "10-10-1955",
                        "jobTitle", "Muppet", "location", "Hollywoord", "phone", "+123456789", "twitterName", "alfresco", "skype",
            ...
            Thread thread = new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    for (int i = 0; i < jobs.size() / 2; i++) {
                        ((ProcessEngineImpl) processEngine).getProcessEngineConfiguration().getClock()
                    ...
                    while (!tasks.isEmpty()) {
                        for (Task task : tasks) {
                            ...
                            ((ProcessEngineImpl) processEngine).getProcessEngineConfiguration().getClock()
                                    .setCurrentTime(new Date(task.getCreateTime().getTime() + random.nextInt(60 * 60 * 1000)));
