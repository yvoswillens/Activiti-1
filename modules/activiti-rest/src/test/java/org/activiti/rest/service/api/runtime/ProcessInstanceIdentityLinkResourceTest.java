...
public class ProcessInstanceIdentityLinkResourceTest extends BaseSpringRestTestCase {
            ...
            if (!link.get("user").isNull()) {
                if (link.get("user").textValue().equals("john")) {
                    ...
                    assertTrue(link
                            ...
                            .textValue()
                            .endsWith(
                                    RestUrls.createRelativeResourceUrl(RestUrls.URL_PROCESS_INSTANCE_IDENTITYLINK, processInstance.getId(),
                                            "john", "customType")));
                    johnFound = true;
                } else {
    ...
    public void testCreateIdentityLink() throws Exception {
        ...
        assertTrue(responseNode
                ...
                .textValue()
                .endsWith(
                        RestUrls.createRelativeResourceUrl(RestUrls.URL_PROCESS_INSTANCE_IDENTITYLINK, processInstance.getId(), "kermit",
                                "myType")));
    ...
    public void testDeleteSingleIdentityLink() throws Exception {
        ...
        closeResponse(executeRequest(
                new HttpDelete(SERVER_URL_PREFIX
                        + RestUrls.createRelativeResourceUrl(RestUrls.URL_PROCESS_INSTANCE_IDENTITYLINK, processInstance.getId(), "kermit",
                                "myType")), HttpStatus.SC_NO_CONTENT));

        // Test with unexisting process identity link
        closeResponse(executeRequest(
                new HttpDelete(SERVER_URL_PREFIX
                        + RestUrls.createRelativeResourceUrl(RestUrls.URL_PROCESS_INSTANCE_IDENTITYLINK, processInstance.getId(), "kermit",
                                "myType")), HttpStatus.SC_NOT_FOUND));

        // Test with unexisting process
        closeResponse(executeRequest(
                new HttpDelete(SERVER_URL_PREFIX
                        + RestUrls.createRelativeResourceUrl(RestUrls.URL_PROCESS_INSTANCE_IDENTITYLINK, "unexistingprocess",
                                RestUrls.SEGMENT_IDENTITYLINKS_FAMILY_USERS, "kermit", "myType")), HttpStatus.SC_NOT_FOUND));
    }
}
