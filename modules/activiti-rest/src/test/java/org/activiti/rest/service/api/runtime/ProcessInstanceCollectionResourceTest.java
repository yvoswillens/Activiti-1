...
public class ProcessInstanceCollectionResourceTest extends BaseSpringRestTestCase {
    ...
    public void testGetProcessInstances() throws Exception {
        ...
        url = RestUrls.createRelativeResourceUrl(RestUrls.URL_PROCESS_INSTANCE_COLLECTION) + "?processDefinitionId="
    ...
    public void testStartProcess() throws Exception {
        ...
        assertTrue(responseNode.get("processDefinitionUrl").asText()
                .endsWith(RestUrls.createRelativeResourceUrl(RestUrls.URL_PROCESS_DEFINITION, processInstance.getProcessDefinitionId())));
        runtimeService.deleteProcessInstance(processInstance.getId(), "testing");

        // Start using process definition id
        requestNode = objectMapper.createObjectNode();
        requestNode.put("processDefinitionId", repositoryService.createProcessDefinitionQuery().processDefinitionKey("processOne")
                .singleResult().getId());
        httpPost.setEntity(new StringEntity(requestNode.toString()));
    ...
    public void testStartProcessUsingKeyAndTenantId() throws Exception {
        ...
        try {
            ...
            tenantDeployment = repositoryService.createDeployment()
                    .addClasspathResource("org/activiti/rest/service/api/oneTaskProcess.bpmn20.xml").tenantId("tenant1").deploy();
