...
public class TaskResourceTest extends BaseSpringRestTestCase {
    ...
    public void testGetProcessTask() throws Exception {
        ...
        assertTrue(responseNode.get("processInstanceUrl").asText()
                .equals(buildUrl(RestUrls.URL_PROCESS_INSTANCE, task.getProcessInstanceId())));
        assertTrue(responseNode.get("processDefinitionUrl").asText()
                .equals(buildUrl(RestUrls.URL_PROCESS_DEFINITION, task.getProcessDefinitionId())));
    ...
    public void testDeleteTask() throws Exception {
        try {
            ...
            // Execute the request
            httpDelete = new HttpDelete(SERVER_URL_PREFIX + RestUrls.createRelativeResourceUrl(RestUrls.URL_TASK, taskId)
                    + "?cascadeHistory=true");
            closeResponse(executeRequest(httpDelete, HttpStatus.SC_NO_CONTENT));
