...
public class TaskAttachmentResourceTest extends BaseSpringRestTestCase {
    ...
    public void testGetAttachment() throws Exception {
        try {
            ...
            assertTrue(responseNode.get("taskUrl").textValue()
            ...
            response = executeRequest(
                    new HttpGet(SERVER_URL_PREFIX
                            + RestUrls.createRelativeResourceUrl(RestUrls.URL_TASK_ATTACHMENT, task.getId(), binaryAttachment.getId())),
                    HttpStatus.SC_OK);
    ...
    public void testGetAttachmentUnexistingTaskAndAttachment() throws Exception {
        try {
            ...
            closeResponse(executeRequest(
                    new HttpGet(SERVER_URL_PREFIX
                            + RestUrls.createRelativeResourceUrl(RestUrls.URL_TASK_ATTACHMENT, "unexistingtask", urlAttachment.getId())),
                    HttpStatus.SC_NOT_FOUND));

            // Get attachment for task attachment
            closeResponse(executeRequest(
                    new HttpGet(SERVER_URL_PREFIX
                            + RestUrls.createRelativeResourceUrl(RestUrls.URL_TASK_ATTACHMENT, task.getId(), "unexistingattachment")),
                    HttpStatus.SC_NOT_FOUND));
    ...
    public void testGetAttachmentContent() throws Exception {
        try {
            ...
            CloseableHttpResponse response = executeRequest(
                    new HttpGet(SERVER_URL_PREFIX
                            + RestUrls.createRelativeResourceUrl(RestUrls.URL_TASK_ATTACHMENT_DATA, task.getId(), binaryAttachment.getId())),
                    HttpStatus.SC_OK);
    ...
    public void testGetAttachmentContentWithMimeType() throws Exception {
        try {
            ...
            // Create Binary-attachment
            Attachment binaryAttachment = taskService.createAttachment("application/xml", task.getId(), null, "Binary attachment",
                    "Binary attachment description", new ByteArrayInputStream("<p>This is binary content</p>".getBytes()));
            taskService.saveAttachment(binaryAttachment);
    ...
    public void testGetAttachmentContentWithoutContent() throws Exception {
        try {
            ...
            closeResponse(executeRequest(
                    new HttpGet(SERVER_URL_PREFIX
                            + RestUrls.createRelativeResourceUrl(RestUrls.URL_TASK_ATTACHMENT_DATA, task.getId(), urlAttachment.getId())),
                    HttpStatus.SC_NOT_FOUND));
