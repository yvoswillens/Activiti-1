...
public class ProcessDefinitionIdentityLinksResourceTest extends BaseSpringRestTestCase {
    ...
    public void testGetIdentityLinksForProcessDefinition() throws Exception {
        ...
        HttpGet httpGet = new HttpGet(SERVER_URL_PREFIX
                + RestUrls.createRelativeResourceUrl(RestUrls.URL_PROCESS_DEFINITION_IDENTITYLINKS_COLLECTION, processDefinition.getId()));
        ...
        for (int i = 0; i < responseNode.size(); i++) {
            ...
            if (!link.get("user").isNull()) {
                ...
                assertTrue(link
                        ...
                        .asText()
                        .endsWith(
                                RestUrls.createRelativeResourceUrl(RestUrls.URL_PROCESS_DEFINITION_IDENTITYLINK, processDefinition.getId(),
                                        "users", "kermit")));
    ...
    public void testGetIdentityLinksForUnexistingProcessDefinition() throws Exception {
        HttpGet httpGet = new HttpGet(SERVER_URL_PREFIX
                + RestUrls.createRelativeResourceUrl(RestUrls.URL_PROCESS_DEFINITION_IDENTITYLINKS_COLLECTION, "unexisting"));
    ...
    public void testGetCandidateStarterFromProcessDefinition() throws Exception {
        ...
        // Get user candidate
        HttpGet httpGet = new HttpGet(SERVER_URL_PREFIX
                + RestUrls.createRelativeResourceUrl(RestUrls.URL_PROCESS_DEFINITION_IDENTITYLINK, processDefinition.getId(), "users",
                        "kermit"));
        CloseableHttpResponse response = executeRequest(httpGet, HttpStatus.SC_OK);
        JsonNode responseNode = objectMapper.readTree(response.getEntity().getContent());
        closeResponse(response);
        ...
        httpGet = new HttpGet(SERVER_URL_PREFIX
                + RestUrls.createRelativeResourceUrl(RestUrls.URL_PROCESS_DEFINITION_IDENTITYLINK, processDefinition.getId(), "groups",
                        "admin"));
