...
public class ActivitiProducer extends DefaultProducer {
    ...
    protected void signal(Exchange exchange) {
        ...
        while (firstTime || (timeout > 0 && (System.currentTimeMillis() - initialTime < timeout))) {
            execution = runtimeService.createExecutionQuery().processDefinitionKey(processKey).processInstanceId(processInstanceId)
                    .activityId(activity).singleResult();

            try {
                Thread.sleep(timeResolution);
            } catch (InterruptedException e) {
                throw new RuntimeException("error occured while waiting for activiti=" + activity + " for processInstanceId="
                        + processInstanceId);
            }
            firstTime = false;
            if (execution != null) {
    ...
    protected String findProcessInstanceId(Exchange exchange) {
        ...
        ProcessInstance processInstance = runtimeService.createProcessInstanceQuery().processInstanceBusinessKey(processInstanceKey)
                .singleResult();
