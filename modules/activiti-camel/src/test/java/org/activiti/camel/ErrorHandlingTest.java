...
public class ErrorHandlingTest extends SpringActivitiTestCase {
    ...
    public void testCamelRouteWorksAsIntended() throws Exception {
        ...
        assertEquals("Process instance not completed", 0,
    ...
    public void testRollbackOnException() throws Exception {
        ...
        assertEquals("No roll-back to previous wait state", 1,
                runtimeService.createExecutionQuery().processInstanceId(processInstance.getId()).activityId(PREVIOUS_WAIT_STATE).count());

        assertEquals("Process instance advanced to next wait state", 0,
                runtimeService.createExecutionQuery().processInstanceId(processInstance.getId()).activityId(NEXT_WAIT_STATE).count());
    }
    ...
    public void testErrorHandledByCamel() throws Exception {
        ...
        assertEquals("Process instance did not reach next wait state", 1,
                runtimeService.createExecutionQuery().processInstanceId(processInstance.getId()).activityId(NEXT_WAIT_STATE).count());
    }
}
